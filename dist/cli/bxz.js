...
var BXZ_RUNTIME = {
  runPy: (script, options = {}) => {
    try {
      const result = (0, import_child_process.execSync)(`python3 -c ${JSON.stringify(script)}`, { stdio: "pipe", encoding: "utf-8" });
      return result.trim();
    } catch (error) {
  ...
  genHtml: async (filePath, content) => {
    try {
      const absolutePath = import_path5.default.resolve(filePath);
      const dir = import_path5.default.dirname(absolutePath);
      await ensureDir(dir);
